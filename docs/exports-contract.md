...
```ts
import type { XlsxColumn } from 'construction-gantt';
...
```ts
import type { Project, Task, CalendarId } from 'construction-gantt';
...
A `construction-gantt-exports` subpackage split is parked. If the seam causes pain for consumers who only need export functionality, we'll revisit.
