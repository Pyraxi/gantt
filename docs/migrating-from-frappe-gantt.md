...
A guide for moving a React app off [frappe-gantt](https://github.com/frappe/gantt) onto `construction-gantt`.

frappe-gantt is a lightweight SVG bar-drawer: **you** compute every task's start and end, frappe draws the bars and emits drag callbacks. `construction-gantt` inverts that — you describe scheduling *intent* (durations, dependencies, constraints, a working-time calendar) and a real engine computes the dates, critical path, and slack for you.
...
| | frappe-gantt | construction-gantt |
|---|---|---|
...
frappe takes a flat `Task[]`. construction-gantt takes a single `Project` that owns tasks, links, calendars, resources, and baselines:

```ts
import type { Project, Task, Link } from 'construction-gantt';
...
| frappe field | construction-gantt | Notes |
|---|---|---|
| `id` | `id` | `string \| number` |
| `name` | `text` | renamed |
...
```ts
import { nzDefaultCalendar, workingMinutesBetween, addWorkingMinutes } from 'construction-gantt';
...
```tsx
import { Gantt } from 'construction-gantt';
...
```tsx
import type { GanttColumn } from 'construction-gantt';
...
frappe emitted `on_date_change(task, start, end)` after a drag and left persistence to you. construction-gantt is the same shape, split by intent. Turn on `editMode` and wire the callbacks to your existing save logic:
...
frappe let you stuff arbitrary keys onto a task object. construction-gantt's `Task` is a closed shape today — there is **no `extra` / passthrough field yet** (it's a planned addition). Until it lands, carry non-scheduling fields one of two ways:

- **Display fields** (trade, status, sub-contractor, cost) → keep them in your own task objects and read them in `GanttColumn.render` via a side-lookup keyed by `task.id`.
- **Round-trip-only fields** (external IDs you never show) → keep a `Map<TaskId, YourMeta>` alongside the `Project`; the engine never needs them.

Both work without engine changes. When the passthrough field ships, this section will be updated.
