...
describe('buildSvarTasks — inline baseline mode', () => {
  test('inline: live row carries baseline_inline, no __baseline_0 phantom', () => {
    let proj = projectOf([task('a'), task('b')]);
    proj = captureBaseline(proj, 0);
    const rows = buildSvarTasks(
      proj.tasks,
      resolveBaselines(proj.baselines, [0]),
      standardCalendar,
      true,
    ...
    expect(rows.some((r) => r.is_baseline_ghost)).toBe(false);
    expect(rows.every((r) => r.baseline_inline !== undefined)).toBe(true);
  });
  test('row mode (default) still emits phantom rows, no baseline_inline', () => {
    let proj = projectOf([task('a')]);
    proj = captureBaseline(proj, 0);
    const rows = buildSvarTasks(
      proj.tasks,
    ...
    expect(rows.filter((r) => r.is_baseline_ghost).length).toBe(1);
    expect(rows[0].baseline_inline).toBeUndefined();
  });
});

describe('ConstructionBar — inline baseline underlay', () => {
  test('renders a positioned underlay div when baseline_inline is set', () => {
    const data = {
      id: 'a',
      text: 'A',
      type: 'task',
      ...
      progress: 0,
      baseline_inline: { leftPct: -20, widthPct: 100 },
    } as never;
    const { container } = render(<ConstructionBar data={data} />);
    const underlay = container.querySelector('.construction-gantt-baseline-inline') as HTMLElement;
    expect(underlay).not.toBeNull();
    expect(underlay.style.left).toBe('-20%');
    expect(underlay.style.width).toBe('100%');
  });
  test('no underlay div when baseline_inline is absent', () => {
    const data = {
      id: 'b',
      text: 'B',
      type: 'task',
    ...
    const { container } = render(<ConstructionBar data={data} />);
    expect(container.querySelector('.construction-gantt-baseline-inline')).toBeNull();
  });
});
...
describe('<Gantt editMode> smoke', () => {
  ...
  test('mounts without crash with onStructuralCommit wired (bolt-in path)', () => {
    const project = projectOf([task('t1'), task('t2')]);
    expect(() =>
      render(<Gantt project={project} editMode onStructuralCommit={() => {}} />),
    ).not.toThrow();
  });
  test('mounts without crash with onTaskSelect wired', () => {
    const project = projectOf([task('t1'), task('t2')]);
    expect(() => render(<Gantt project={project} onTaskSelect={() => {}} />)).not.toThrow();
  });
  test('mounts without crash with onTaskEditRequest wired', () => {
    const project = projectOf([task('t1'), task('t2')]);
    expect(() => render(<Gantt project={project} onTaskEditRequest={() => {}} />)).not.toThrow();
  });
  test('mounts with boldSummaryRows toggled off', () => {
    const project = projectOf([task('t1'), task('t2')]);
    expect(() => render(<Gantt project={project} boldSummaryRows={false} />)).not.toThrow();
  });
  test('injects a bold-summary <style> targeting the summary row by default', () => {
    const summary: Task = { ...task('p'), type: 'summary' };
    const child: Task = { ...task('c'), parent: 'p' };
    const { container } = render(<Gantt project={projectOf([summary, child])} />);
    const styleText = [...container.querySelectorAll('style')].map((s) => s.textContent).join('\n');
    expect(styleText).toContain('.wx-row[data-id=":p"]');
    expect(styleText).toContain('font-weight:600');
    expect(styleText).not.toContain('.wx-row[data-id=":c"]'); // leaf row not bolded
  });
  test('mounts with a custom-editor column + onCellEdit wired', () => {
    const project = projectOf([task('t1'), task('t2')]);
    expect(() =>
      render(
        <Gantt
          ...
          onCellEdit={() => {}}
          columns={[
            { id: 'text', header: 'Name', field: 'text' },
            {
              id: 'trade',
              header: 'Trade',
              render: (() => null) as never,
              editor: { type: 'text', getValue: () => '' },
            },
          ]}
        />,
      ),
  ...
  test('boldSummaryRows={false} omits the bold-summary style', () => {
    const summary: Task = { ...task('p'), type: 'summary' };
    const child: Task = { ...task('c'), parent: 'p' };
    const { container } = render(
      <Gantt project={projectOf([summary, child])} boldSummaryRows={false} />,
    );
    const styleText = [...container.querySelectorAll('style')].map((s) => s.textContent).join('\n');
    expect(styleText).not.toContain('font-weight:600');
  });
  test('mounts with tooltip={false} (tooltip disabled)', () => {
    const project = projectOf([task('t1'), task('t2')]);
    expect(() => render(<Gantt project={project} tooltip={false} />)).not.toThrow();
  });
  test('mounts in editMode with long labels (label-overflow measurement hooks)', () => {
    const long: Task = {
      ...task('t1', 'A very long task name that will not fit inside a narrow bar'),
    };
    expect(() =>
      render(
        <Gantt project={projectOf([long, task('t2')])} editMode onStructuralCommit={() => {}} />,
      ),
    ).not.toThrow();
  });
  test('mounts with a custom tooltip fn', () => {
    const project = projectOf([task('t1'), task('t2')]);
    expect(() =>
      render(<Gantt project={project} tooltip={(t) => <span>{t.text}</span>} />),
    ).not.toThrow();
  });
  test('mounts with baselineRender="inline"', () => {
    let project = projectOf([task('t1'), task('t2')]);
    project = captureBaseline(project, 0);
    expect(() =>
      render(<Gantt project={project} baselineIndices={[0]} baselineRender="inline" />),
    ).not.toThrow();
  });
});
...
describe('ConstructionTooltip — built-in default', () => {
  const base = {
    id: 'a',
    text: 'Pour foundation',
    type: 'task' as const,
  ...
  test('shows title, date range, working days, status', () => {
    const { container } = render(<ConstructionTooltip data={base} />);
    const t = container.textContent ?? '';
    expect(t).toContain('Pour foundation');
    expect(t).toContain('6 working days');
    expect(t).toContain('Not started');
  });
  test('progress 100 → Complete; partial → In progress (n%)', () => {
    expect(
      render(<ConstructionTooltip data={{ ...base, progress: 100 }} />).container.textContent,
    ).toContain('Complete');
    expect(
      render(<ConstructionTooltip data={{ ...base, progress: 40 }} />).container.textContent,
    ).toContain('In progress (40%)');
  });
  test('assignee pulled from extra (assignedSubName), critical + past-deadline flags', () => {
    const { container } = render(
      <ConstructionTooltip
        data={{
          ...base,
          extra: { assignedSubName: 'ZZ Builder' },
          is_critical: true,
          deadline_missed: true,
        }}
      />,
    ...
    const t = container.textContent ?? '';
    expect(t).toContain('Assignee: ZZ Builder');
    expect(t).toContain('On critical path');
    expect(t).toContain('Past deadline');
  });
  test('milestone → single date, no working-days/status line', () => {
    const { container } = render(<ConstructionTooltip data={{ ...base, type: 'milestone' }} />);
    const t = container.textContent ?? '';
    expect(t).not.toContain('working');
    expect(t).not.toContain('Not started');
  });
  test('singular day label', () => {
    expect(
      render(<ConstructionTooltip data={{ ...base, duration: 1 }} />).container.textContent,
    ).toContain('1 working day');
  });
});

describe('ConstructionBar — label overflow beside the bar (BT parity)', () => {
  const barData = {
    id: 'x',
    text: 'A task name too long for a narrow bar',
    type: 'task' as const,
    start: new Date(2026, 0, 5),
  ...
  // happy-dom has no layout (scrollWidth/clientWidth = 0), so stub them to force
  // the overflow branch deterministically.
  function withMeasuredWidths(scroll: number, client: number, fn: () => void) {
  ...
  test('overflowing label → renders a beside-bar label; inside label hidden', () => {
    withMeasuredWidths(500, 100, () => {
      const { container } = render(<ConstructionBar data={barData} />);
      const beside = container.querySelector('.construction-gantt-bar-label-out');
      expect(beside).not.toBeNull();
      expect(beside?.textContent).toBe(barData.text);
      expect(getComputedStyle(beside as Element).position).toBe('absolute');
      // the in-bar copy stays mounted but hidden (stable measurement)
      const inside = [...container.querySelectorAll('span')].find(
        (s) => s !== beside && s.textContent === barData.text,
      );
      expect(inside && getComputedStyle(inside).visibility).toBe('hidden');
    });
  });

  test('fitting label → no beside-bar label, inside stays visible', () => {
    withMeasuredWidths(100, 100, () => {
      const { container } = render(<ConstructionBar data={barData} />);
      expect(container.querySelector('.construction-gantt-bar-label-out')).toBeNull();
    });
  });
});
