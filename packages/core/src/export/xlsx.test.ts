...
/** Read a Blob back into an array-of-arrays for assertions (test-only helper). */
async function readBlobToAoa(
  blob: Blob,
): Promise<{ sheetNames: string[]; firstSheet: unknown[][] }> {
  const wb = new ExcelJS.Workbook();
  await wb.xlsx.load(await blob.arrayBuffer());
  const sheetNames = wb.worksheets.map((w) => w.name);
  const ws = wb.worksheets[0];
  const firstSheet: unknown[][] = [];
  ws?.eachRow((row) => {
    // ExcelJS `row.values` is 1-indexed with a leading empty slot — drop it.
    const vals = (row.values as unknown[]).slice(1);
    firstSheet.push(vals);
  });
  return { sheetNames, firstSheet };
}
...
describe('exportXLSX — Blob roundtrip', () => {
  test('returns a Blob that xlsx can read back with the right shape', async () => {
    ...
    const { sheetNames, firstSheet } = await readBlobToAoa(blob);
    expect(sheetNames).toEqual(['Programme']);

    expect(firstSheet).toHaveLength(3);
    expect(firstSheet[0][0]).toBe('ID');
    expect(firstSheet[1][0]).toBe('a');
    expect(firstSheet[2][0]).toBe('b');
  });
  ...
  test('respects a custom sheetName', async () => {
    ...
    const { sheetNames } = await readBlobToAoa(blob);
    expect(sheetNames).toEqual(['Programme v2']);
  });
});
