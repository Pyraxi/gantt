// Carriage-guarantee tests for `Task.extra` (CM carry-through bag).
//
// `extra` is a consumer-owned opaque bag the engine never reads or writes
// (per ADR-005 — surface the carry-through *pattern*, not consumer semantics).
// Coord acked the carriage mechanism on 2026-06-14 (the actuals/WBS *display*
// surface stays parked for CM sign-off). These three tests lock the contract
// from the proposal `docs/cm-task-extra-carry-through.md`:
//
...
// A representative CM carry-through bag: host-app fields gantt doesn't model.
