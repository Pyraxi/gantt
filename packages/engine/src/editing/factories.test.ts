...
import type { Project, Task, TaskSegment } from '../types.js';
import {
  CreateLinkCommand,
  DeleteLinkCommand,
  DeleteTaskCommand,
...
describe('factories — task commands', () => {
  ...
  test('createTask returns a create-task EditCommand that adds the task', () => {
    // createTask now returns a unified EditCommand wrapper (not a bare
    // CreateTaskCommand instance) so it can also promote a leaf parent to
    // summary on the add-child path (P1.1). Assert the contract, not the class.
    const cmd = createTask(sampleTask);
    expect(cmd.kind).toBe('create-task');
    ...
    expect(cmd.apply(p).tasks.map((t) => t.id)).toEqual([sampleTask.id]);
  });
