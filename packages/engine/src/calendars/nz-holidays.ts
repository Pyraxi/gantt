...
function regionalAnniversary(year: number, region: NZRegion): CalendarException | null {
  switch (region) {
    ...
    case 'westland':
      // Monday NEAREST 1 December (employment.govt.nz), not the first Monday of
      // December — the two diverge when 1 Dec falls Tue–Thu (e.g. 2022/2026/2027).
      return makeException(nearestMondayTo(year, 11, 1), 'Westland Anniversary');
