...
// ---------------------------------------------------------------------------
// Backward pass across SS/FF/SF + negative-lag (lead) links
// ---------------------------------------------------------------------------

function link(type: Link['type'], source: string, target: string, lag = 0): Link {
  return { id: `${source}-${target}-${type}`, source, target, type, lag };
}

describe('schedule: backward pass SS/FF/SF (all link types)', () => {
  test('SS chain: both tasks critical, no phantom slack', () => {
    const a = task('a', STD_DAY_MINS);
    const b = task('b', STD_DAY_MINS);
    const result = schedule(project([a, b], [link('SS', 'a', 'b')]));
    for (const t of result.tasks) expect(t.computed!.totalSlack).toBe(0);
  });

  test('FF chain: both tasks critical, no phantom slack', () => {
    const a = task('a', STD_DAY_MINS);
    const b = task('b', STD_DAY_MINS);
    const result = schedule(project([a, b], [link('FF', 'a', 'b')]));
    for (const t of result.tasks) expect(t.computed!.totalSlack).toBe(0);
  });

  test('SF chain: predecessor is not spuriously negative', () => {
    const a = task('a', STD_DAY_MINS);
    const b = task('b', STD_DAY_MINS);
    const result = schedule(project([a, b], [link('SF', 'a', 'b')]));
    const aC = result.tasks.find((t) => t.id === 'a')!.computed!;
    // A drives B's finish; A cannot be negative on a feasible schedule.
    expect(aC.totalSlack).toBeGreaterThanOrEqual(0);
  });
});

describe('schedule: negative (lead) lag does not fabricate negative slack', () => {
  test('FS with -240min lead: predecessor slack is 0, not -240 (regression)', () => {
    // A(1d) —FS lag -240min→ B(1d). Forward overlaps B into A; the backward pass
    // must add the lead back (sign-aware subtract), not drop it. Pre-fix:
    // A.totalSlack = -240, isCritical spuriously true.
    const a = task('a', STD_DAY_MINS);
    const b = task('b', STD_DAY_MINS);
    const result = schedule(project([a, b], [link('FS', 'a', 'b', -240)]));
    const aC = result.tasks.find((t) => t.id === 'a')!.computed!;
    expect(aC.totalSlack).toBe(0);
    expect(aC.freeSlack).toBe(0);
  });

  test('SS with negative lead lag: no false negative slack on the predecessor', () => {
    const a = task('a', STD_DAY_MINS);
    const b = task('b', STD_DAY_MINS);
    const result = schedule(project([a, b], [link('SS', 'a', 'b', -120)]));
    const aC = result.tasks.find((t) => t.id === 'a')!.computed!;
    expect(aC.totalSlack).toBeGreaterThanOrEqual(0);
  });
});
...
// ---------------------------------------------------------------------------
// Free slack (previously asserted nowhere in the package)
// ---------------------------------------------------------------------------

describe('schedule: freeSlack', () => {
  test('FS chain A→B→C: every non-sink task has zero free slack', () => {
    const a = task('a', STD_DAY_MINS);
    const b = task('b', STD_DAY_MINS);
    const c = task('c', STD_DAY_MINS);
    const result = schedule(project([a, b, c], [fs('a', 'b'), fs('b', 'c')]));
    expect(result.tasks.find((t) => t.id === 'a')!.computed!.freeSlack).toBe(0);
    expect(result.tasks.find((t) => t.id === 'b')!.computed!.freeSlack).toBe(0);
  });

  test('diamond short branch: B can slip a full day without delaying D (free slack = 1 day)', () => {
    // A → B(1d) → D ; A → C(2d) → D. B is on the shorter branch.
    const a = task('a', STD_DAY_MINS);
    const b = task('b', STD_DAY_MINS);
    const c = task('c', 2 * STD_DAY_MINS);
    const d = task('d', STD_DAY_MINS);
    const links: Link[] = [fs('a', 'b'), fs('a', 'c'), fs('b', 'd'), fs('c', 'd')];
    const result = schedule(project([a, b, c, d], links));
    expect(result.tasks.find((t) => t.id === 'b')!.computed!.freeSlack).toBe(STD_DAY_MINS);
    expect(result.tasks.find((t) => t.id === 'c')!.computed!.freeSlack).toBe(0);
  });

  test('negative lead lag: free slack is 0, not -240 (regression)', () => {
    const a = task('a', STD_DAY_MINS);
    const b = task('b', STD_DAY_MINS);
    const result = schedule(project([a, b], [link('FS', 'a', 'b', -240)]));
    expect(result.tasks.find((t) => t.id === 'a')!.computed!.freeSlack).toBe(0);
