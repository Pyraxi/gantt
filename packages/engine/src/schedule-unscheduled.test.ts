...
describe('schedule() — unscheduled tasks', () => {
  ...
  test('re-scheduling prior output: marking a scheduled task unscheduled strips its stale computed (idempotence)', () => {
    // CM's edit round-trip shape: schedule() → edit an output task → schedule() again.
    const project: Project = {
      start: START,
      defaultCalendarId: 'std',
      tasks: [t('a'), t('b')],
    ...
    const first = schedule(project);
    const bFirst = first.tasks.find((x) => x.id === 'b');
    // Precondition: b was scheduled and carries computed (incl. criticality).
    expect(bFirst?.computed).toBeDefined();

    // Feed the prior OUTPUT back in, but now mark b unscheduled.
    const roundTrip: Project = {
      ...project,
      tasks: first.tasks.map((x) => (x.id === 'b' ? { ...x, unscheduled: true } : x)),
    };
    const second = schedule(roundTrip);
    const bSecond = second.tasks.find((x) => x.id === 'b');
    // The stale computed from the first run must not survive.
    expect(bSecond?.computed).toBeUndefined();
  });
});
